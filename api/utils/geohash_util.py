...
def validate_criteria(new_query_criteria, require_center_and_radius):
    ...
    elif require_center_and_radius and (
            new_query_criteria.get("center", False) or new_query_criteria.get("radius", False)):
...
def geohash_query(geohash, bits):
    ...
    if len(geohash) < precision:
        print(
            "geohash.length < precision: " + len(geohash) + " < " + precision + " bits= " + bits + " g_BITS_PER_CHAR= "
            + g_BITS_PER_CHAR)
    ...
    if end_value >= len(g_BASE32):
        print("endValue > 31: endValue=" + str(end_value) + " < " + str(precision) + " bits=" + str(
            bits) + " g_BITS_PER_CHAR="
              + str(g_BITS_PER_CHAR))
...
def geohash_queries(center, radius):
    ...
    for idx, query in enumerate(queries):
        if not any(idx > other_idx and query[0] == other[0] and query[1] == other[1] for (other_idx, other) in
                   enumerate(queries)):
