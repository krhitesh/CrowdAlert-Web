...
describe('renders', ()  => {
    test('renders test component when user is logged in', () => {
        const wrapper = setup({}, { auth: { isLoggedIn: true } }).dive();
        expect(findByTestAttr(wrapper, 'component-child-component').length).toBe(1);
    });

    test('renders test component when user is logged in', () => {
        const wrapper = setup({}, { auth: { isLoggedIn: false } }).dive();
        expect(findByTestAttr(wrapper, 'component-redirect').length).toBe(1);
    });
});
